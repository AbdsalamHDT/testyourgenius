/* ================================================================
   tyg-data.js — Shared Data, Scoring, Archetype Maps & Utilities
   Used by both assessment.js and result.js
   ================================================================ */
...
const RESULT_STORAGE_KEY = "tyg_result_data";
...
function computeResults(answers) {
  ...
  QUESTIONS.forEach(q => {
    const val = answers[q.id]; if (val == null) return;
...
// ── RESULT DATA PERSISTENCE (bridge between assessment → result page) ──

function saveResultData(answers) {
  try {
    localStorage.setItem(RESULT_STORAGE_KEY, JSON.stringify({ answers: answers, timestamp: Date.now() }));
  } catch (e) { /* silent */ }
}
...
function clearResultData() {
  try { localStorage.removeItem(RESULT_STORAGE_KEY); } catch (e) { /* silent */ }
}
